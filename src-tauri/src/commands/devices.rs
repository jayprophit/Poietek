...
mod desktop {
    ...
    fn audio_device(
        ...
        let (supported_configs, capability_status, capability_message) = match configs {
            ...
            Err(error) => (
                ...
                "probe_error",
                Some(format!(
                    "Supported configurations could not be read: {error}"
                )),
            ),
        };
            ...
            let host = match cpal::host_from_id(host_id) {
                ...
                Err(error) => {
                    warnings.push(format!(
                        "Audio host {host_name} could not start: {error}"
                    ));
                    continue;
                }
            };
    ...
    fn scan_midi(
        warnings: &mut Vec<String>,
    ) -> (Vec<NativeMidiPort>, Vec<NativeMidiPort>) {
        let mut inputs = Vec::new();
        let mut outputs = Vec::new();
        ...
        match MidiInput::new("Poietek Studio device inventory") {
            ...
            Err(error) => {
                warnings.push(format!(
                    "MIDI input inventory is unavailable: {error}"
                ));
            }
        }
        ...
        match MidiOutput::new("Poietek Studio device inventory") {
            ...
            Err(error) => {
                warnings.push(format!(
                    "MIDI output inventory is unavailable: {error}"
                ));
            }
        }
    ...
    pub(super) fn inventory() -> NativeStudioDeviceInventory {
        let mut warnings = Vec::new();

        let (audio_hosts, audio_inputs, audio_outputs) =
            scan_audio(&mut warnings);

        let (midi_inputs, midi_outputs) =
            scan_midi(&mut warnings);
...
pub(super) fn inventory() -> NativeStudioDeviceInventory {
    NativeStudioDeviceInventory {
        ...
        warnings: vec![
            "Use the platform permission and device adapters planned for the mobile engine slice."
                .to_string(),
        ],
    }
}
