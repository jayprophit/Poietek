...
export interface WavPcm16Format {
  container: "wav";
  codec: "linear_pcm";
  bitDepth: 16;
  endianness: "little";
...
export interface WavEncodingProgress {
  phase: "encoding" | "complete";
  completedFrames: number;
  totalFrames: number;
  ratio: number;
}

export interface EncodeWavPcm16Options {
  signal?: AbortSignal;
  onProgress?: (progress: WavEncodingProgress) => void;
  framesPerChunk?: number;
  yieldControl?: () => Promise<void>;
}

export interface WavPcm16Result {
  blob: Blob;
  format: WavPcm16Format;
  frameCount: number;
  durationSeconds: number;
  clippedSampleCount: number;
  replacedNonFiniteSampleCount: number;
}

export class WavEncodingCancelledError extends Error {
  readonly code = "WAV_ENCODING_CANCELLED";
 ...
 */
export async function encodeWavPcm16(
  source: PcmAudioSource,
  options: EncodeWavPcm16Options = {},
): Promise<WavPcm16Result> {
  ...
  if (!Number.isSafeInteger(dataBytes) || riffPayloadBytes > 0xffff_ffff) {
    throw new RangeError(
      "The rendered audio is too large for the 32-bit RIFF/WAVE container.",
    );
  }
  ...
  for (let firstFrame = 0; firstFrame < frameCount; firstFrame += framesPerChunk) {
    ...
    if (lastFrame < frameCount) await yieldControl();
  }
...
function assertPcmSource(source: PcmAudioSource): void {
  ...
  if (source.channels.length !== 1 && source.channels.length !== 2) {
    throw new RangeError(
      "PCM WAV export currently supports explicit mono or stereo sources only.",
    );
  }
...
function throwIfCancelled(signal: AbortSignal | undefined): void {
  if (signal?.aborted) throw new WavEncodingCancelledError();
}
