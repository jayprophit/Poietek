...
export function validateProject(project: PoietekProject): string[] {
  ...
  if (
    !Number.isFinite(project.settings.tuning.referenceHz) ||
  ...
  for (const asset of project.assets) {
    ...
    if (!asset.contentHash) issues.push(`Asset ${asset.id} requires a content hash.`);
  ...
  for (const track of project.tracks) {
    ...
    for (const clip of track.clips) {
      ...
      if (
        !Number.isFinite(clip.fadeInSeconds) ||
