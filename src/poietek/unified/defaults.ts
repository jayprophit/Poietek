...
const POLICY_KINDS: Array<[GovernancePolicyKind, string]> = [
...
export interface UnifiedProductionSuiteOptions {
  projectId: string;
  localActorId: string;
  sampleRate?: number;
...
export function createLocalUnifiedProductionSuite(
  options: UnifiedProductionSuiteOptions,
): UnifiedProductionSuite {
  const now = options.now ?? new Date().toISOString();
  const localModel = (id: string) =>
    availableCapability(id, 'poietek.unified-project-model.v1', now, 'local');
  const configure = (id: string) =>
    requiresConfigurationCapability(
      id,
      'No reviewed production provider or operational service is configured.',
    );
  const unavailable = (id: string, message: string) =>
    unavailableCapability(id, 'NOT_IMPLEMENTED', message);

  return {
    schemaVersion: UNIFIED_PRODUCTION_SCHEMA_VERSION,
    ...
    timeline: {
      ...
      score: {
        ...
        notationCapability: unavailable(
          'unified.notation_editor',
          'The notation engraving and score editor is not implemented.',
        ),
        cues: [],
      },
      picture: {
        timelineModelCapability: localModel('unified.picture_timeline_model'),
        decodeCapability: unavailable(
          'unified.picture_decode',
          'No reviewed video decode adapter is running.',
        ),
        renderCapability: unavailable(
          'unified.picture_render',
          'No reviewed video/VFX renderer is running.',
        ),
        clips: [],
        captionAssetIds: [],
        vfxGraphIds: [],
    ...
    community: {
      ...
      endToEndEncryptionCapability: unavailable(
        'community.end_to_end_encryption',
        'No audited end-to-end messaging protocol is implemented.',
      ),
    ...
    governance: {
      jurisdictionAssessmentCapability: configure(
        'governance.jurisdiction_assessment',
      ),
      ageAssuranceCapability: configure('governance.age_assurance'),
      moderationOperationsCapability: configure(
        'governance.moderation_operations',
      ),
      appealsCapability: configure('governance.appeals'),
      dataRightsCapability: configure('governance.data_rights'),
      transparencyReportingCapability: configure(
        'governance.transparency_reporting',
      ),
