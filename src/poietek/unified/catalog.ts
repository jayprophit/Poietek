import type {GovernancePolicyKind, MarketplaceCategory} from './contracts';

export type DeliveryState = 'working_model' | 'foundation' | 'planned' | 'external_gate';
...
export const UNIFIED_CREATOR_STAGES: UnifiedCatalogItem[] = [
...
export const POIETEK_TV_CATALOG: UnifiedCatalogItem[] = [
...
export const COMMUNITY_MODE_CATALOG: UnifiedCatalogItem[] = [
...
].map(([id, label]) => ({id: id as MarketplaceCategory, label}));

export const GOVERNANCE_POLICY_CATALOG: Array<{kind: GovernancePolicyKind; title: string; purpose: string}> = [
...
].map(([kind, title, purpose]) => ({kind: kind as GovernancePolicyKind, title, purpose}));

export const HELP_FAQ_CATALOG = [
...
export const COMPLIANCE_BASELINE_CATALOG = [
