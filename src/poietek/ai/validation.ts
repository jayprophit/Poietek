import {AI_SETTINGS_SCHEMA_VERSION, type AiProviderConfiguration, type AiSettingsDocument, type AiValidationIssue, type AiValidationResult} from './contracts';
import {getAiProviderDefinition} from './catalog';
...
export function validateAiProviderConfiguration(configuration: AiProviderConfiguration): AiValidationResult {
  const issues: AiValidationIssue[] = [];
  if (!configuration.id.trim()) issues.push({path: 'id', message: 'Configuration id is required.'});
  if (!configuration.displayName.trim()) issues.push({path: 'displayName', message: 'Display name is required.'});
  const definition = getAiProviderDefinition(configuration.provider);
...
export function validateAiSettings(document: AiSettingsDocument): AiValidationResult {
  const issues: AiValidationIssue[] = [];
  if (document.schemaVersion !== AI_SETTINGS_SCHEMA_VERSION) issues.push({path: 'schemaVersion', message: 'Unsupported AI settings schema.'});
  ...
  document.configurations.forEach((configuration, index) => {
    if (ids.has(configuration.id)) issues.push({path: `configurations.${index}.id`, message: 'Configuration ids must be unique.'});
...
export function isLoopbackAiEndpoint(value: string): boolean {
