export const GENERATIVE_AUDIO_SCHEMA_VERSION = '1.0.0' as const;

export type GenerativeAudioProviderId =
  | 'suno-external'
  | 'udio-external'
  | 'stability-audio-api'
  | 'loudly-api'
  ...
  | 'sound-protocol-reference'
  | 'bittensor-subnet'
  | 'custom-audio-model';

export type GenerativeAudioWorkflowId =
  | 'idea_variations'
  | 'sample_forge'
  | 'section_extend'
  | 'region_recompose'
  | 'owned_audio_variation'
  | 'adaptive_score'
  | 'lyrics_to_demo'
  | 'stem_assist'
  | 'accompaniment_assist';

export type GenerativeAudioExecution =
  | 'external_product'
  | 'secure_proxy'
  | 'localhost_sidecar'
  | 'native_sidecar'
  | 'decentralized_gateway';

export type GenerativeAudioIntegrationState =
  | 'official_api_candidate'
  | 'local_sidecar_candidate'
  | 'external_product_only'
  | 'reference_only'
  | 'decentralized_connector_candidate'
  | 'custom_adapter';

export interface GenerativeAudioWorkflow {
  id: GenerativeAudioWorkflowId;
  name: string;
  purpose: string;
  requiresSourceAudio: boolean;
  supportsSourceAudio: boolean;
  preservesSourceAudio: true;
  outputRule: string;
}

export interface GenerativeAudioProviderDefinition {
  id: GenerativeAudioProviderId;
  name: string;
  family: 'commercial_api' | 'local_open_model' | 'external_product' | 'community_protocol' | 'decentralized_network' | 'custom';
  integrationState: GenerativeAudioIntegrationState;
  execution: readonly GenerativeAudioExecution[];
  workflows: readonly GenerativeAudioWorkflowId[];
  officialUrl: string;
  summary: string;
  credentialRule: 'none' | 'secure_store_or_server' | 'external_account' | 'wallet_and_subnet_specific' | 'adapter_defined';
  defaultEnabled: false;
  notes: readonly string[];
}

export interface GenerativeAudioRightsDeclaration {
  sourceAudio: 'not_used' | 'not_attested' | 'user_attested';
  voiceIdentity: 'not_used' | 'not_attested' | 'user_attested';
  creativeDirection: 'not_reviewed' | 'reviewed_no_named_imitation' | 'blocked';
  acknowledgement: string | null;
}

export interface GenerativeAudioDraft {
  schemaVersion: typeof GENERATIVE_AUDIO_SCHEMA_VERSION;
  id: string;
  projectId: string;
  providerId: GenerativeAudioProviderId;
  workflowId: GenerativeAudioWorkflowId;
  prompt: string;
  lyrics: string | null;
  ...
  sectionEndSeconds: number | null;
  variance: number;
  remoteConsent: boolean;
  rights: GenerativeAudioRightsDeclaration;
  createdAt: string;
}

export interface GenerativeAudioRouteState {
  providerId: GenerativeAudioProviderId;
  state: 'unavailable' | 'configuration_required' | 'ready' | 'error';
  enabled: boolean;
  endpoint: string | null;
  model: string | null;
  credentialReference: string | null;
  termsEvidenceReference: string | null;
  checkedAt: string | null;
  message: string;
}

export interface GenerativeAudioValidationIssue {
  path: string;
  severity: 'blocking' | 'attention';
  message: string;
}

export interface GenerativeAudioPlan {
  draftId: string;
  providerId: GenerativeAudioProviderId;
  state: 'draft' | 'blocked' | 'configuration_required' | 'ready_to_queue';
  safeToQueue: boolean;
  issues: GenerativeAudioValidationIssue[];
...
export interface GenerativeAudioOutputEvidence {
  generationId: string;
  providerId: GenerativeAudioProviderId;
  model: string;
  ...
  workflowId: GenerativeAudioWorkflowId;
  sourceAssetIds: string[];
  promptRecord: 'stored_locally' | 'redacted' | 'provider_only';
  providerTermsEvidenceReference: string | null;
  licenceEvidenceReference: string | null;
  aiGenerated: true;
  originalityVerified: false;
  ownershipDetermined: false;
}

export const GENERATIVE_AUDIO_WORKFLOWS: readonly GenerativeAudioWorkflow[] = [
...
export const GENERATIVE_AUDIO_PROVIDERS: readonly GenerativeAudioProviderDefinition[] = [
  ...
  {id: 'custom-audio-model', name: 'Custom audio model', family: 'custom', integrationState: 'custom_adapter', execution: ['localhost_sidecar', 'native_sidecar', 'secure_proxy'], workflows: GENERATIVE_AUDIO_WORKFLOWS.map((item) => item.id), officialUrl: 'https://github.com/jayprophit/Studio-Daw-Station-SDS-', summary: 'Versioned adapter point for a user-selected local, hosted or specialist audio model.', credentialRule: 'adapter_defined', defaultEnabled: false, notes: ['Must declare exact inputs, outputs, licence, data use and hardware limits', 'No project mutation without preview and user acceptance']},
] as const;

export function getGenerativeAudioProvider(id: GenerativeAudioProviderId): GenerativeAudioProviderDefinition {
  const provider = GENERATIVE_AUDIO_PROVIDERS.find((item) => item.id === id);
  if (!provider) throw new Error(`Unknown generative audio provider: ${id}`);
  return provider;
}

export function getGenerativeAudioWorkflow(id: GenerativeAudioWorkflowId): GenerativeAudioWorkflow {
  const workflow = GENERATIVE_AUDIO_WORKFLOWS.find((item) => item.id === id);
  if (!workflow) throw new Error(`Unknown generative audio workflow: ${id}`);
  return workflow;
}

export function createGenerativeAudioDraft(
  projectId: string,
  now = new Date(),
): GenerativeAudioDraft {
  return {
    schemaVersion: GENERATIVE_AUDIO_SCHEMA_VERSION,
...
export function createUnavailableGenerativeAudioRoute(
  providerId: GenerativeAudioProviderId,
): GenerativeAudioRouteState {
...
export function validateGenerativeAudioDraft(
  draft: GenerativeAudioDraft,
  route: GenerativeAudioRouteState = createUnavailableGenerativeAudioRoute(draft.providerId),
): GenerativeAudioValidationIssue[] {
  const issues: GenerativeAudioValidationIssue[] = [];
  const provider = getGenerativeAudioProvider(draft.providerId);
  const workflow = getGenerativeAudioWorkflow(draft.workflowId);
  if (draft.schemaVersion !== GENERATIVE_AUDIO_SCHEMA_VERSION) issues.push({path: 'schemaVersion', severity: 'blocking', message: 'Unsupported generative-audio draft schema.'});
  if (!draft.projectId.trim()) issues.push({path: 'projectId', severity: 'blocking', message: 'A canonical project is required.'});
  if (!draft.prompt.trim()) issues.push({path: 'prompt', severity: 'blocking', message: 'Describe the production purpose and musical direction.'});
  if (draft.prompt.length > 4000) issues.push({path: 'prompt', severity: 'blocking', message: 'Prompt must be 4,000 characters or fewer.'});
  if (!Number.isFinite(draft.durationSeconds) || draft.durationSeconds < 1 || draft.durationSeconds > 600) issues.push({path: 'durationSeconds', severity: 'blocking', message: 'Duration must be between 1 and 600 seconds.'});
  if (!Number.isInteger(draft.variationCount) || draft.variationCount < 1 || draft.variationCount > 8) issues.push({path: 'variationCount', severity: 'blocking', message: 'Variation count must be an integer from 1 to 8.'});
  if (!Number.isFinite(draft.variance) || draft.variance < 0 || draft.variance > 1) issues.push({path: 'variance', severity: 'blocking', message: 'Variance must be between 0 and 1.'});
  if (!provider.workflows.includes(draft.workflowId)) issues.push({path: 'workflowId', severity: 'blocking', message: `${provider.name} does not declare support for ${workflow.name}.`});
  if (workflow.requiresSourceAudio && draft.sourceAssetIds.length === 0) issues.push({path: 'sourceAssetIds', severity: 'blocking', message: `${workflow.name} requires an explicitly selected source asset.`});
  if (draft.sourceAssetIds.length > 0 && draft.rights.sourceAudio !== 'user_attested') issues.push({path: 'rights.sourceAudio', severity: 'blocking', message: 'The user must attest that they may send and transform every selected source asset.'});
  if (draft.sourceAssetIds.length === 0 && draft.rights.sourceAudio !== 'not_used') issues.push({path: 'rights.sourceAudio', severity: 'attention', message: 'No source audio is selected; source-audio attestation is unnecessary.'});
  if (draft.workflowId === 'lyrics_to_demo' && !draft.lyrics?.trim()) issues.push({path: 'lyrics', severity: 'blocking', message: 'Lyrics-to-Demo requires creator-supplied lyrics.'});
  if (draft.rights.creativeDirection !== 'reviewed_no_named_imitation') issues.push({path: 'rights.creativeDirection', severity: 'blocking', message: 'Review the direction and remove requests to imitate a named living artist or protected recording.'});
  if (draft.rights.voiceIdentity === 'not_attested') issues.push({path: 'rights.voiceIdentity', severity: 'blocking', message: 'A voice reference requires explicit identity consent evidence.'});
  if (provider.execution.some((item) => item === 'secure_proxy' || item === 'external_product' || item === 'decentralized_gateway') && !draft.remoteConsent) issues.push({path: 'remoteConsent', severity: 'blocking', message: 'This route can send data off-device and requires consent for this request.'});
  if (route.providerId !== draft.providerId) issues.push({path: 'route.providerId', severity: 'blocking', message: 'The selected runtime route does not match the draft provider.'});
  if (route.state !== 'ready' || !route.enabled) issues.push({path: 'route.state', severity: 'blocking', message: route.message});
  if (route.state === 'ready' && !route.termsEvidenceReference) issues.push({path: 'route.termsEvidenceReference', severity: 'blocking', message: 'A reviewed provider/model terms evidence reference is required before queueing.'});
  return issues;
}

export function planGenerativeAudioDraft(
  draft: GenerativeAudioDraft,
  route: GenerativeAudioRouteState = createUnavailableGenerativeAudioRoute(draft.providerId),
): GenerativeAudioPlan {
...
export function validateGenerativeAudioCatalog(): string[] {
  const issues: string[] = [];
  const ids = new Set<string>();
  for (const provider of GENERATIVE_AUDIO_PROVIDERS) {
    if (ids.has(provider.id)) issues.push(`Duplicate provider id: ${provider.id}`);
    ids.add(provider.id);
    if (provider.defaultEnabled !== false) issues.push(`${provider.id} must be disabled by default`);
    if (!provider.officialUrl.startsWith('https://')) issues.push(`${provider.id} lacks a secure official reference`);
    if (provider.integrationState === 'reference_only' && provider.workflows.length > 0) issues.push(`${provider.id} is reference-only but declares generation workflows`);
  }
  return issues;
}
