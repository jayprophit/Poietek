...
function capabilityIsUsable(report: HardwareCapabilityReport): boolean {
...
function validateCapability(
  ...
  path: string,
  issues: HardwareValidationIssue[],
): void {
  if (!hasText(report.capabilityId)) {
    issues.push(
      issue("CAPABILITY_ID_REQUIRED", `${path}.capabilityId`, "A capability report requires an id."),
    );
  }
    ...
    if (!hasText(report.implementationId) || !isTimestamp(report.observedAt)) {
      issues.push(
        issue(
          ...
          path,
          "Available or degraded hardware capabilities require an implementation and observation timestamp.",
        ),
      );
    }
    if (report.evidence === null || report.source === "unknown") {
      issues.push(
        issue(
          ...
          path,
          "Available or degraded hardware capabilities require adapter or physical measurement evidence.",
        ),
      );
    }
  }
  ...
  if (report.source === "adapter" && report.evidence?.kind !== "adapter_negotiation") {
    issues.push(
      issue(
        ...
        `${path}.evidence`,
        "An adapter-sourced capability requires adapter negotiation evidence.",
      ),
    );
  }
  if (
    report.source === "physical_measurement" &&
    ...
    issues.push(
      issue(
        ...
        `${path}.evidence`,
        "A physically measured capability requires physical loopback evidence.",
      ),
    );
  }
  if (
    ...
    issues.push(
      issue(
        "NOT_MEASURED_HAS_EVIDENCE",
        path,
        "A not-measured state cannot contain measurement evidence or an observation time.",
      ),
    );
  }
}
...
function endpointProblem(
  foundation: HardwareFoundation,
  endpoint: HardwareEndpointRef,
): "device" | "profile" | "port" | "channel" | null {
  const device = foundation.devices.find((candidate) => candidate.id === endpoint.deviceId);
  if (!device) return "device";
  if (!device.selectedProfile) return "profile";
...
function validateEndpoint(
  ...
  if (problem) {
    issues.push(
      issue(
        "HARDWARE_ENDPOINT_INVALID",
        `${path}.${problem}`,
        "The endpoint must reference a selected device profile, declared port, and valid one-based channel.",
      ),
    );
  }
}

function findLoopbackEvidence(
  foundation: HardwareFoundation,
  evidenceId: string,
): PhysicalLoopbackEvidence | undefined {
    ...
    if (!hasText(value.id) || seen.has(value.id)) {
      issues.push(
        issue(
          "HARDWARE_ID_INVALID",
          `${path}[${index}].id`,
          "Hardware record ids must be non-empty and unique within their collection.",
        ),
      );
    }
    seen.add(value.id);
...
export function validateHardwareFoundation(
  foundation: HardwareFoundation,
): HardwareValidationIssue[] {
  const issues: HardwareValidationIssue[] = [];

  if (foundation.schemaVersion !== HARDWARE_FOUNDATION_SCHEMA_VERSION) {
    issues.push(
      issue(
        "HARDWARE_SCHEMA_UNSUPPORTED",
        "schemaVersion",
        `Expected hardware schema ${HARDWARE_FOUNDATION_SCHEMA_VERSION}.`,
      ),
    );
  }
  ...
  if (!Number.isInteger(foundation.revision) || foundation.revision < 0) {
    issues.push(
      issue(
        "HARDWARE_REVISION_INVALID",
        "revision",
        "Hardware revision must be a non-negative integer.",
      ),
    );
  }
  if (!isTimestamp(foundation.createdAt) || !isTimestamp(foundation.updatedAt)) {
    issues.push(
      issue(
        "HARDWARE_TIMESTAMP_INVALID",
        "createdAt",
        "Hardware creation and update times must be valid timestamps.",
      ),
    );
  }
  ...
  validateUniqueIds(
    foundation.timing.metering.observations,
    "timing.metering.observations",
    issues,
  );
  ...
  foundation.profiles.forEach((profile, profileIndex) => {
    ...
    profile.ports.forEach((port, portIndex) => {
      if (!Number.isInteger(port.channelCount) || port.channelCount < 1) {
        issues.push(
          issue(
            "PORT_CHANNEL_COUNT_INVALID",
            `${path}.ports[${portIndex}].channelCount`,
            "Hardware ports require a positive integer channel count.",
          ),
        );
      }
    });
    if (
      ...
      issues.push(
        issue(
          "PROFILE_VERIFICATION_EVIDENCE_REQUIRED",
          `${path}.provenance`,
          "A verified profile requires a source, SHA-256 digest and verification time.",
        ),
      );
    }
  });
  ...
  foundation.devices.forEach((device, deviceIndex) => {
    ...
    if (device.selectedProfile) {
      ...
      if (!profile) {
        issues.push(
          issue(
            ...
            `${path}.selectedProfile.profileId`,
            "The selected device profile is not present in this foundation.",
          ),
        );
      } else {
        const expectedDigest =
          profile.provenance.verification === "verified"
            ...
            : null;
        if (
          selected.verificationAtSelection !== profile.provenance.verification ||
          selected.profileDigestAtSelection !== expectedDigest
        ) {
          issues.push(
            issue(
              "PROFILE_SELECTION_PROVENANCE_MISMATCH",
              `${path}.selectedProfile`,
              "The selection must snapshot the selected profile's verification and digest.",
            ),
          );
        }
      }
      if (!isTimestamp(selected.selectedAt) || !hasText(selected.selectedByActorId)) {
        issues.push(
          issue(
            "PROFILE_SELECTION_USER_EVIDENCE_REQUIRED",
            `${path}.selectedProfile`,
            "Profiles must be selected by an identified user action at a valid time.",
          ),
        );
      }
    }
    if (
      (device.connection.state === "connected" || device.connection.state === "degraded") &&
      (!hasText(device.connection.adapterId) || !isTimestamp(device.connection.observedAt))
    ) {
      issues.push(
        issue(
          "DEVICE_CONNECTION_OBSERVATION_REQUIRED",
          `${path}.connection`,
          "A connected device requires an adapter and observation time.",
        ),
      );
    }
    device.lastObservedCapabilities.forEach((report, reportIndex) =>
      validateCapability(report, `${path}.lastObservedCapabilities[${reportIndex}]`, issues),
    );
  });
  ...
  foundation.loopbackEvidence.forEach((evidence, evidenceIndex) => {
    ...
    if (
      !isPositiveFinite(evidence.sampleRateHz) ||
      ...
      issues.push(
        issue(
          "LOOPBACK_EVIDENCE_INCOMPLETE",
          path,
          "Loopback evidence requires endpoints, sample rate, repetitions, procedure, actor and time.",
        ),
      );
    }
  });
  ...
  foundation.patchRoutes.forEach((route, routeIndex) => {
    ...
    if (
      route.verification.state === "physical_loopback_verified" &&
      !findLoopbackEvidence(foundation, route.verification.evidenceId)
    ) {
      issues.push(
        issue(
          "PATCH_LOOPBACK_EVIDENCE_MISSING",
          `${path}.verification`,
          "A physically verified patch must reference retained loopback evidence.",
        ),
      );
    }
  });
  ...
  foundation.analogueInserts.forEach((insert, insertIndex) => {
    ...
    if (insert.roundTripLatency.state === "measured") {
      ...
      if (
        ...
        issues.push(
          issue(
            "LATENCY_PHYSICAL_EVIDENCE_REQUIRED",
            `${path}.roundTripLatency`,
            "Measured insert latency requires matching loopback evidence and sample-derived timing.",
          ),
        );
      }
    }
    if (insert.levelCalibration.state === "measured") {
      ...
      if (
        ...
        issues.push(
          issue(
            "ANALOGUE_CALIBRATION_EVIDENCE_REQUIRED",
            `${path}.levelCalibration`,
            "Measured analogue calibration requires retained physical level evidence.",
          ),
        );
      }
    }
  ...
  if (
    timing.audioTransport.positionObservation?.precision === "audio_callback_observed" &&
    !hasText(timing.audioTransport.positionObservation.adapterId)
  ) {
    issues.push(
      issue(
        "AUDIO_CALLBACK_ADAPTER_REQUIRED",
        "timing.audioTransport.positionObservation",
        "Audio-callback position observations require the producing adapter id.",
      ),
    );
  }
  if (
    timing.sampleClock.lockState !== "not_measured" &&
    (!capabilityIsUsable(timing.sampleClock.capability) ||
      !isTimestamp(timing.sampleClock.observedAt))
  ) {
    issues.push(
      issue(
        "SAMPLE_CLOCK_LOCK_EVIDENCE_REQUIRED",
        "timing.sampleClock",
        "A sample-clock lock claim requires a negotiated capability and observation time.",
      ),
    );
  }
  if (
    (timing.sampleClock.measuredSampleRateHz !== null ||
    ...
    issues.push(
      issue(
        "SAMPLE_CLOCK_MEASUREMENT_EVIDENCE_REQUIRED",
        "timing.sampleClock",
        "Measured sample rate or drift requires an observed clock capability.",
      ),
    );
  }
  if (
    !["not_measured", "inactive"].includes(timing.midiClock.syncState) &&
    (!capabilityIsUsable(timing.midiClock.capability) ||
      !isTimestamp(timing.midiClock.observedAt))
  ) {
    issues.push(
      issue(
        "MIDI_CLOCK_SYNC_EVIDENCE_REQUIRED",
        "timing.midiClock",
        "MIDI clock synchronization requires a negotiated capability and observation time.",
      ),
    );
  }
  if (
    timing.wordClock.lockState !== "not_measured" &&
    (!capabilityIsUsable(timing.wordClock.capability) ||
      !isTimestamp(timing.wordClock.observedAt))
  ) {
    issues.push(
      issue(
        "WORD_CLOCK_LOCK_EVIDENCE_REQUIRED",
        "timing.wordClock",
        "Word-clock lock requires a negotiated capability and observation time.",
      ),
    );
  }
  ...
  timing.timecode.forEach((timecode, timecodeIndex) => {
    ...
    if (
      ...
      issues.push(
        issue(
          "TIMECODE_LOCK_EVIDENCE_REQUIRED",
          path,
          "Locked LTC or MTC requires capability evidence, frame rate, address and time.",
        ),
      );
    }
  });
  ...
  if (timing.metering.observations.length > 0 && !capabilityIsUsable(timing.metering.capability)) {
    issues.push(
      issue(
        "METERING_CAPABILITY_REQUIRED",
        "timing.metering.observations",
        "Meter observations require a negotiated or physically measured metering capability.",
      ),
    );
  }
  ...
  foundation.digitalConsoleSync.forEach((sync, syncIndex) => {
    ...
    if (sync.connection.state === "connected" && !capabilityIsUsable(sync.capability)) {
      issues.push(
        issue(
          "CONSOLE_SYNC_CAPABILITY_REQUIRED",
          path,
          "Connected digital-console control sync requires a negotiated capability.",
        ),
      );
    }
  });
  ...
  foundation.analogueConsoles.forEach((consoleRecord, consoleIndex) => {
    ...
    validateCapability(
      consoleRecord.physicalAutomationCapability,
      `${path}.physicalAutomationCapability`,
      issues,
    );
    if (
      consoleRecord.lastRecall.state === "user_confirmed" &&
      (!hasText(consoleRecord.lastRecall.confirmedByActorId) ||
        !isTimestamp(consoleRecord.lastRecall.confirmedAt))
    ) {
      issues.push(
        issue(
          "ANALOGUE_RECALL_CONFIRMATION_REQUIRED",
          `${path}.lastRecall`,
          "Analogue recall is only confirmed by an identified user at a valid time.",
        ),
      );
    }
    consoleRecord.insertIds.forEach((insertId, insertIndex) => {
      if (!foundation.analogueInserts.some((insert) => insert.id === insertId)) {
        issues.push(
          issue(
            "ANALOGUE_INSERT_MISSING",
            `${path}.insertIds[${insertIndex}]`,
            "The analogue console references an insert that is not retained.",
          ),
        );
      }
    });
