...
function validateCatalogItem(
  ...
  if (!original || original.kind !== "creator_original") {
    issues.push(
      issue(
        "ORIGINAL_RENDITION_REQUIRED",
        base,
        "Catalog item must reference one creator-original rendition.",
      ),
    );
  }
  if (item.visibility === "circle" && item.circleIds.length === 0) {
    issues.push(
      issue(
        "CIRCLE_ID_REQUIRED",
        `${base}.circleIds`,
        "Circle visibility requires at least one circle id.",
      ),
    );
  }
  if (
    item.publicationState === "published_remotely" &&
    item.moderation.state === "pending"
  ) {
    issues.push(
      issue(
        "REMOTE_PUBLICATION_UNVERIFIED",
        base,
        "Remote publication cannot be inferred while moderation is pending.",
      ),
    );
  }
  if (
    item.moderation.state !== "not_requested" &&
    item.moderation.state !== "pending" &&
    ...
    issues.push(
      issue(
        ...
        `${base}.moderation`,
        "A moderation decision requires authority, reference and observation evidence.",
      ),
    );
  }
  item.renditions.forEach((rendition, renditionIndex) => {
    if (rendition.kind !== "time_preserving_derivative") return;
    if (rendition.derivedFromRenditionId !== item.originalRenditionId) {
      issues.push(
        issue(
          "DERIVATIVE_SOURCE_INVALID",
          `${base}.renditions[${renditionIndex}]`,
          "A compatibility rendition must derive from the creator original.",
        ),
      );
    }
    if (
      ...
      issues.push(
        issue(
          "DERIVATIVE_DURATION_CHANGED",
          `${base}.renditions[${renditionIndex}]`,
          "A time-preserving derivative must retain the original duration.",
        ),
      );
    }
    if (
      !hasText(rendition.generation.backendId) ||
      !hasText(rendition.generation.requestId) ||
      !validTime(rendition.generation.renderedAt)
    ) {
      issues.push(
        issue(
          "DERIVATIVE_DSP_EVIDENCE_REQUIRED",
          `${base}.renditions[${renditionIndex}].generation`,
          "A derivative requires real DSP backend and render evidence.",
        ),
      );
    }
  });
...
export function validateCommunityHub(
  ...
  hub.federationEndpoints.forEach((endpoint, index) => {
    if (isCapabilityUsable(endpoint.capability) && !hasText(endpoint.endpointUrl)) {
