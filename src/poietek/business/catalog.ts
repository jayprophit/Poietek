import {
  BUSINESS_TIER_SCHEMA_VERSION,
  type BusinessTierDefinition,
  type BusinessTierGovernance,
  type BusinessTierId,
  type ReferencePrice,
...
const entitlement = (
  id: string,
  label: string,
  state: TierEntitlement["state"],
  notes: string,
  limit: TierEntitlement["limit"] = null,
  externalGates: string[] = [],
): TierEntitlement => ({id, label, state, notes, limit, externalGates});

export const BUSINESS_TIER_GOVERNANCE: BusinessTierGovernance = {
  schemaVersion: BUSINESS_TIER_SCHEMA_VERSION,
...
const deviceLimit: TierEntitlement["limit"] = {kind: "device_resource_limited", enforcement: "local"};
const fairUse: TierEntitlement["limit"] = {kind: "subject_to_fair_use", enforcement: "provider_required", policyStatus: "not_defined"};

export const BUSINESS_TIER_CATALOG: readonly BusinessTierDefinition[] = [
  {
    id: "free",
    order: 1,
    name: "Free Forever",
...
export function resolveTierEntitlements(
  tierId: BusinessTierId,
  catalog: readonly BusinessTierDefinition[] = BUSINESS_TIER_CATALOG,
): TierEntitlement[] {
  const tiers = new Map(catalog.map((tier) => [tier.id, tier]));
  const visiting = new Set<BusinessTierId>();
  const resolved = new Map<string, TierEntitlement>();
  const addTier = (id: BusinessTierId) => {
    if (visiting.has(id)) throw new Error(`Tier inheritance cycle at ${id}.`);
    const tier = tiers.get(id);
    if (!tier) throw new Error(`Unknown business tier ${id}.`);
    visiting.add(id);
    if (tier.inheritsFrom) addTier(tier.inheritsFrom);
...
export function searchBusinessTiers(query: string) {
  const tokens = query.toLocaleLowerCase().split(/[^\p{L}\p{N}]+/u).filter((token) => token.length > 1);
  return BUSINESS_TIER_CATALOG.filter((tier) => {
...
const pounds = (minorUnits: string) => `£${(Number(minorUnits) / 100).toLocaleString("en-GB", {maximumFractionDigits: 2})}`;

export function formatReferencePrice(price: ReferencePrice): string {
  if (price.kind === "free") return "Reference £0";
  if (price.kind === "custom") return "Reference custom quote";
  ...
  if (price.kind === "range" && price.minimumMinorUnits && price.maximumMinorUnits) return `Reference ${pounds(price.minimumMinorUnits)}–${pounds(price.maximumMinorUnits)}${cadence}${seat}`;
  if (price.kind === "starting_at" && price.minimumMinorUnits) return `Reference from ${pounds(price.minimumMinorUnits)}${cadence}${seat}`;
