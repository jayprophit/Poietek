export type VisionCapabilityStatus = 'operational' | 'foundation' | 'planned' | 'blocked_external';
export type VisionHorizon = 'now' | 'next' | 'later';

export interface VisionCapabilityArea {
  id: string;
  category: string;
  name: string;
  purpose: string;
...
export const CORE_ECOSYSTEM_PILLARS: readonly EcosystemPillar[] = [
...
export const SDS_VISION_CATALOG: readonly VisionCapabilityArea[] = [
  {
...
export function summarizeVisionCatalog(areas: readonly VisionCapabilityArea[] = SDS_VISION_CATALOG) {
  return areas.reduce<Record<VisionCapabilityStatus, number>>((summary, area) => {
    summary[area.status] += 1;
    return summary;
  }, {operational: 0, foundation: 0, planned: 0, blocked_external: 0});
}

export function searchVisionCatalog(query: string, areas: readonly VisionCapabilityArea[] = SDS_VISION_CATALOG) {
  const tokens = query.toLocaleLowerCase().split(/[^\p{L}\p{N}]+/u).filter((token) => token.length > 1);
  if (!tokens.length) return [...areas];
