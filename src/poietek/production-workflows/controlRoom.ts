...
const minimumOutputChannels: Readonly<Record<MonitorFormat, number>> = {
  mono: 1,
  stereo: 2,
  '5.1': 6,
  '7.1.4': 12,
...
export function createControlRoomState(
  overrides: Partial<ControlRoomState> = {},
): ControlRoomState {
...
export function validateControlRoomState(state: ControlRoomState): void {
  if (state.schemaVersion !== '1.0.0') throw new Error('Unsupported control-room schema.');
  ...
  if (!state.routeObservation) return;
...
export function evaluateControlRoom(state: ControlRoomState): ControlRoomReport {
  validateControlRoomState(state);
