...
function validateMidiMessage(message: MidiNoteMessage): void {
...
export function validateMidiTransformRule(rule: MidiTransformRule): void {
...
export function transformMidiNoteMessage(
  message: MidiNoteMessage,
  rule: MidiTransformRule,
): MidiTransformResult {
  validateMidiMessage(message);
  validateMidiTransformRule(rule);
  if (rule.bypass) return {kind: 'forward', message: {...message}};
