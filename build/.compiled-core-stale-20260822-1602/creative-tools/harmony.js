...
exports.createHarmonyPadBank = createHarmonyPadBank;
...
function createHarmonyPadBank(input) {
    const padCount = input.padCount ?? 16;
    if (!Number.isInteger(input.rootMidiNote) || input.rootMidiNote < 0 || input.rootMidiNote > 127)
        throw new RangeError('Root MIDI note must be between 0 and 127.');
    if (!Number.isInteger(padCount) || padCount < 1 || padCount > 128)
        throw new RangeError('Pad count must be between 1 and 128.');
    ...
    return { ...input, schemaVersion: contracts_1.CREATIVE_TOOLKIT_SCHEMA_VERSION, pads };
}
