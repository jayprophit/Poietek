...
exports.analyzeBasicAudioHealth = analyzeBasicAudioHealth;
function db(value) {
...
function analyzeChannel(samples) {
    ...
    for (const sample of samples) {
        ...
        if (absolute >= 1)
            clipped += 1;
        if (absolute >= nearClip && !near)
            nearClipEventCount += 1;
...
function analyzeBasicAudioHealth(channels) {
    if (!channels.length)
    ...
    const stereoCorrelation = channels.length >= 2 ? correlation(channels[0], channels[1]) : null;
    const recommendations = [];
    let status = "good";
    if (clipped > 0) {
        status = "critical";
        recommendations.push("Full-scale samples were detected and may indicate digital clipping. Inspect the waveform and lower the analogue/input gain before another take; lowering a later digital fader cannot restore clipped peaks.");
    }
    else if (peak >= -1) {
        status = "warning";
        recommendations.push("Peak headroom is very small. Lower the input/preamp slightly and retest the loudest expected passage.");
    }
    else if (peak >= -6) {
        status = "advisory";
        recommendations.push("The recording is healthy but has limited peak headroom. Consider a slightly lower input level for unpredictable sources.");
    }
    if (maxDc > 0.01) {
