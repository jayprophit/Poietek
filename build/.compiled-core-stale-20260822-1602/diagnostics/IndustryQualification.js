...
exports.INDUSTRY_QUALIFICATION_ASSESSMENTS = exports.INDUSTRY_QUALIFICATION_LANES = exports.INDUSTRY_REFERENCE_PLATFORMS = void 0;
exports.assessQualificationLane = assessQualificationLane;
exports.summarizeIndustryQualification = summarizeIndustryQualification;
exports.searchIndustryQualification = searchIndustryQualification;
const points = {
    verified: 100,
    working: 75,
    foundation: 40,
...
const criterion = (id, title, state, evidence, fiveStarExit, mandatory = true) => ({ id, title, state, mandatory, evidence, fiveStarExit });
const lane = (id, kind, order, name, purpose, peerIds, criteria) => ({ id, kind, order, name, purpose, peerIds, criteria });
exports.INDUSTRY_REFERENCE_PLATFORMS = [
    {
        id: 'ableton-live',
        name: 'Ableton Live',
...
const systemLanes = [
    lane('system-professional-daw', 'system', 1, 'Professional DAW', 'Record, arrange, edit, mix, master and deliver complex sessions reliably.', ['ableton-live', 'logic-pro', 'pro-tools', 'fl-studio'], [
        ...
        criterion('native-audio', 'Native low-latency engine', 'external_gate', ['Browser Web Audio is working; the native shell now inventories CPAL audio endpoints and advertised configurations without opening a stream or inventing latency.'], 'Pass measured device round-trip, dropout, clock, multichannel and sustained-load acceptance on supported operating systems.'),
    ...
    lane('system-midi-hub', 'system', 4, 'MIDI Hub', 'Route, transform, clock and monitor MIDI devices and protocols.', ['ableton-live', 'logic-pro', 'mpc'], [
        criterion('web-midi', 'Honest MIDI discovery and events', 'working', ['Permission, unsupported, denied and error states are explicit; SysEx requires opt-in. Desktop native builds also inventory real midir ports at startup without opening them.'], 'Add explicit native input connections and output scheduling, then prove hot-plug, large-port routing and browser/platform compatibility.'),
...
const volumeImplementationState = [
...
const volumeLanes = volumeNames.map((name, index) => {
    const number = index + 1;
    const key = String(number).padStart(2, '0');
    const implementationState = volumeImplementationState[index];
    return lane(`volume-${key}`, 'volume', number, name, `Professional Volume ${key} controls the requirements, evidence, decisions and release gates for ${name.toLowerCase()}.`, volumePeers[index], [
...
exports.INDUSTRY_QUALIFICATION_LANES = [
    ...systemLanes,
    ...volumeLanes,
];
function assessQualificationLane(definition) {
...
exports.INDUSTRY_QUALIFICATION_ASSESSMENTS = exports.INDUSTRY_QUALIFICATION_LANES.map(assessQualificationLane);
function summarizeIndustryQualification(assessments = exports.INDUSTRY_QUALIFICATION_ASSESSMENTS) {
...
function searchIndustryQualification(query, kind = 'all') {
    const normalized = query.trim().toLowerCase();
    const references = new Map(exports.INDUSTRY_REFERENCE_PLATFORMS.map((item) => [item.id, item]));
    return exports.INDUSTRY_QUALIFICATION_ASSESSMENTS.filter((item) => {
