...
exports.scoreStudioBenchmark = scoreStudioBenchmark;
exports.runStudioBenchmark = runStudioBenchmark;
function metricScore(metric, thresholds, higherIsBetter) {
    if (metric.state !== "measured" || metric.value == null || !Number.isFinite(metric.value))
        return 0;
    ...
    if (value >= normalized[3])
        return 100;
    if (value >= normalized[2])
        return 80;
...
function scoreStudioBenchmark(metrics) {
    const components = [
...
async function runDspBenchmark() {
...
async function runSchedulerBenchmark() {
    const intervalMs = 10;
    const samples = [];
    let previous = now();
...
async function runOfflineAudioBenchmark() {
...
async function runIndexedDbBenchmark() {
