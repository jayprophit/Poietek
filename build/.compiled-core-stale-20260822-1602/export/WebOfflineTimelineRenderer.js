...
exports.WebOfflineTimelineRenderer = exports.OfflineTimelineRenderError = void 0;
const validate_1 = require("../domain/validate");
const tempo_1 = require("../timeline/tempo");
class OfflineTimelineRenderError extends Error {
    code;
...
exports.OfflineTimelineRenderError = OfflineTimelineRenderError;
class WebOfflineTimelineRenderer {
    assetStore;
    contextFactory;
    constructor(assetStore, contextFactory = defaultOfflineAudioContextFactory()) {
    ...
    async render(project, options = {}) {
        ...
        if (capability.state === "unavailable") {
            throw new OfflineTimelineRenderError(capability.unavailableCode, capability.reason);
        }
        const validationIssues = (0, validate_1.validateProject)(project);
        if (validationIssues.length) {
            throw new Error(`Cannot render an invalid project: ${validationIssues.join(" | ")}`);
        }
        ...
        if (!playable.length) {
            throw new OfflineTimelineRenderError("EMPTY_TIMELINE", "The project contains no audible audio clips to render.");
        }
        const timelineEndSeconds = Math.max(...playable.map((item) => item.clipStartSeconds + item.timelineDurationSeconds));
        ...
        try {
            ...
            throw new OfflineTimelineRenderError("OFFLINE_AUDIO_CONTEXT_UNAVAILABLE", "The platform rejected the requested offline audio context.", { cause });
        }
        ...
        for (const item of playable) {
            ...
            if (!scheduleClip(context, item, buffer)) {
                throw new Error(`Clip ${item.clip.id} has no decodable source range to render.`);
            }
        }
        options.onProgress?.({
            phase: "rendering",
...
exports.WebOfflineTimelineRenderer = WebOfflineTimelineRenderer;
function collectPlayableClips(project) {
    ...
    for (const track of project.tracks) {
        ...
        for (const clip of track.clips) {
            ...
            const clipEndSeconds = (0, tempo_1.ticksToSeconds)(clip.startTick + clip.durationTicks, project.tempoMap, project.settings.ppq);
...
function scheduleClip(context, item, buffer) {
    const sourceOffsetSeconds = item.clip.sourceOffsetSeconds;
    const declaredRemaining = item.clip.sourceDurationSeconds === null
        ? Number.POSITIVE_INFINITY
        : item.clip.sourceDurationSeconds;
    const durationSeconds = Math.min(item.timelineDurationSeconds, declaredRemaining, buffer.duration - sourceOffsetSeconds);
    if (!Number.isFinite(sourceOffsetSeconds) ||
...
function throwIfRenderCancelled(signal) {
    if (signal?.aborted) {
        throw new OfflineTimelineRenderError("RENDER_CANCELLED", "Offline timeline rendering was cancelled.");
    }
}
function defaultOfflineAudioContextFactory() {
    if (typeof OfflineAudioContext === "undefined")
        return null;
    return (channelCount, frameCount, sampleRate) => new OfflineAudioContext(channelCount, frameCount, sampleRate);
}
