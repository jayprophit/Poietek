...
function searchCreativeOs(foundation, query, limit = 30) {
    ...
    for (const node of foundation.graph.nodes) {
        ...
        if (score > 0)
            results.push({ id: `node:${node.id}`, kind: node.kind, label: node.label, excerpt: node.tags.join(' · '), score, sourceId: node.id });
    }
    for (const item of foundation.journal) {
        const score = scoreText(queryTokens, item.title, item.body);
        if (score > 0)
            results.push({ id: `journal:${item.id}`, kind: 'journal', label: item.title, excerpt: item.body.slice(0, 180), score, sourceId: item.id });
    }
    for (const item of foundation.annotations) {
        const score = scoreText(queryTokens, item.modality, item.body);
        if (score > 0)
            results.push({ id: `annotation:${item.id}`, kind: 'annotation', label: `${item.modality} annotation`, excerpt: item.body.slice(0, 180), score, sourceId: item.id });
    }
