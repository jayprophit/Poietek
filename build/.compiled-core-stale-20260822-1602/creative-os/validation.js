...
exports.validateCreativeOsFoundation = validateCreativeOsFoundation;
const contracts_1 = require("./contracts");
function issue(code, path, message) {
    return { code, path, message };
}
function validateCreativeOsFoundation(value) {
    const issues = [];
    if (value.schemaVersion !== contracts_1.CREATIVE_OS_SCHEMA_VERSION) {
        issues.push(issue('UNSUPPORTED_VERSION', 'schemaVersion', 'The Creative OS schema version is unsupported.'));
    }
    if (!value.projectId)
        issues.push(issue('PROJECT_REQUIRED', 'projectId', 'A project id is required.'));
    if (!value.localActorId)
        issues.push(issue('ACTOR_REQUIRED', 'localActorId', 'A local actor id is required.'));
    ...
    const unique = (id, path) => {
        if (!id)
            issues.push(issue('ID_REQUIRED', path, 'An id is required.'));
        else if (ids.has(id))
            issues.push(issue('DUPLICATE_ID', path, `Duplicate id ${id}.`));
        else
            ids.add(id);
    };
    value.creatorIdentities.forEach((creator, index) => {
        ...
        if (!creator.displayName.trim())
            issues.push(issue('NAME_REQUIRED', `creatorIdentities.${index}.displayName`, 'A display name is required.'));
    ...
    value.assets.forEach((asset, assetIndex) => {
        ...
        if (asset.projectId !== value.projectId)
            issues.push(issue('ASSET_PROJECT_MISMATCH', `assets.${assetIndex}.projectId`, 'The asset belongs to another project.'));
        if (!asset.contentHash.trim())
            issues.push(issue('HASH_REQUIRED', `assets.${assetIndex}.contentHash`, 'Content identity requires a hash.'));
        ...
        asset.replicas.forEach((replica, replicaIndex) => {
            ...
            if (replica.contentHash !== asset.contentHash)
                issues.push(issue('REPLICA_HASH_MISMATCH', `assets.${assetIndex}.replicas.${replicaIndex}.contentHash`, 'Replica identity must match the asset hash.'));
    ...
    value.annotations.forEach((annotation, index) => {
        ...
        if (!nodeIds.has(annotation.targetNodeId))
            issues.push(issue('ANNOTATION_TARGET_MISSING', `annotations.${index}.targetNodeId`, 'Annotation target is missing from the graph.'));
        if (!annotation.body.trim())
            issues.push(issue('ANNOTATION_BODY_REQUIRED', `annotations.${index}.body`, 'Annotation body is required.'));
        if (annotation.startSeconds !== null && annotation.startSeconds < 0)
            issues.push(issue('ANNOTATION_TIME_INVALID', `annotations.${index}.startSeconds`, 'Annotation time cannot be negative.'));
        if (annotation.endSeconds !== null && (annotation.startSeconds === null || annotation.endSeconds < annotation.startSeconds))
            issues.push(issue('ANNOTATION_RANGE_INVALID', `annotations.${index}.endSeconds`, 'Annotation end must follow its start.'));
    ...
    value.journal.forEach((entry, index) => {
        ...
        for (const nodeId of entry.linkedNodeIds)
            if (!nodeIds.has(nodeId))
                issues.push(issue('JOURNAL_NODE_MISSING', `journal.${index}.linkedNodeIds`, `Journal node ${nodeId} is missing.`));
    });
    value.intent.rules.forEach((rule, index) => {
        unique(rule.id, `intent.rules.${index}.id`);
        if (!rule.trait.trim() || !rule.reason.trim())
            issues.push(issue('INTENT_DETAIL_REQUIRED', `intent.rules.${index}`, 'Intent rules require a trait and creator reason.'));
    });
    ...
    if (JSON.stringify(summary) !== JSON.stringify(calculated))
        issues.push(issue('PREFLIGHT_SUMMARY_MISMATCH', 'preflight.summary', 'Preflight summary must be derived from its findings.'));
    value.preflight.findings.forEach((finding, index) => {
        ...
        if (finding.state === 'pass' && finding.evidenceRefs.length === 0)
            issues.push(issue('PASS_EVIDENCE_REQUIRED', `preflight.findings.${index}.evidenceRefs`, 'A passed check requires evidence.'));
    });
    if (value.storagePolicy.minimumLocalReplicas < 1)
        issues.push(issue('LOCAL_REPLICA_REQUIRED', 'storagePolicy.minimumLocalReplicas', 'At least one local replica is required.'));
    if (!value.storagePolicy.allowProviderStorage && value.storagePolicy.minimumRemoteReplicas > 0)
        issues.push(issue('REMOTE_POLICY_CONFLICT', 'storagePolicy.minimumRemoteReplicas', 'Remote replicas cannot be required while provider storage is disabled.'));
    value.handoffs.forEach((handoff, index) => {
        unique(handoff.id, `handoffs.${index}.id`);
        if (handoff.projectId !== value.projectId)
            issues.push(issue('HANDOFF_PROJECT_MISMATCH', `handoffs.${index}.projectId`, 'Handoff belongs to another project.'));
        if (!Number.isInteger(handoff.projectRevision) || handoff.projectRevision < 0 || handoff.playheadSamples < 0)
            issues.push(issue('HANDOFF_POSITION_INVALID', `handoffs.${index}`, 'Handoff revision and position must be non-negative.'));
        handoff.pluginRequirements.forEach((plugin, pluginIndex) => {
            if (plugin.state === 'unsupported' && !plugin.statePreserved)
                issues.push(issue('PLUGIN_STATE_MUST_BE_PRESERVED', `handoffs.${index}.pluginRequirements.${pluginIndex}`, 'Unsupported plugin state must remain preserved.'));
            if (plugin.state === 'unsupported' && !plugin.fallbackAssetId)
                issues.push(issue('PLUGIN_FALLBACK_REQUIRED', `handoffs.${index}.pluginRequirements.${pluginIndex}`, 'Unsupported plugins require a freeze/render fallback asset.'));
        });
    });
    return issues;
}
