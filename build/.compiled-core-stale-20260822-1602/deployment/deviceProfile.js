...
exports.deriveDeviceRuntimeProfile = deriveDeviceRuntimeProfile;
...
function deriveInputMode(input) {
    const touch = input.coarsePointer || input.maxTouchPoints > 0;
    if (touch && input.finePointer)
        return 'hybrid';
    if (touch)
        return 'touch';
    if (input.finePointer || input.hover)
        return 'mouse-keyboard';
    return 'unknown';
}
function deriveDeviceClass(input, width, height) {
    if (!width)
        return 'other';
    ...
    if (touchDevice && (width < 680 || shortestSide < 600))
        return 'mobile';
    if (width < 1180 && touchDevice)
        return 'tablet';
    if (input.finePointer || width >= 1180)
        return 'desktop';
    return 'other';
}
function deriveLayout(deviceClass, width) {
    if (deviceClass === 'mobile' || (width > 0 && width < 680))
        return 'handheld';
    if (deviceClass === 'desktop' && width >= 1180)
        return 'expanded';
    return 'compact';
}
function deriveDeviceRuntimeProfile(input) {
